...
var compileTest = new Bach.Action.ToolRunner(new Bach.Command("javac").add("-d").add(target + "/test").add("-cp").add(List.of(Path.of(target + "/main"), junit)).addAllJavaFiles(List.of(Path.of("src/test"))))
var test = new Bach.Action.ToolRunner(new Bach.Command("java").add("-ea").add("-cp").add(List.of(Path.of(target + "/test"), Path.of(target + "/main"), junit)).add("org.junit.platform.console.ConsoleLauncher").add("--scan-class-path"))
